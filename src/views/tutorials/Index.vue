<template>
  <div class="flex flex-col items-center">
    <div class="w-full flex flex-col items-start" v-if="showSearch">
      ...
      <core-button v-if="searchTerm !== ''" class="mt-4" @click="clearSearch"
        >Limpiar búsqueda</core-button
      >
    </div>
    <div class="w-full">
      <tutorials-empty-state
        v-if="initialLoad && tutorials.length === 0"
      ></tutorials-empty-state>
      <tutorials-list
...
export default {
  ...
  computed: {
    showSearch() {
      return (
        (this.searchTerm.trim() === "" && this.tutorials.length > 0) ||
